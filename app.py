...
def echo_explorer(path: str = ""):
    ...
    try:
        ...
        # MODIFIED: Return dict
        return {"files": [{"name": c.path, "type": c.type} for c in contents]}
    except Exception as e:
        # MODIFIED: Return dict for error
        return {"error": f"⚠️ Грешка при изследване: {str(e)}"}
...
def echo_reader(file_path: str):
    ...
    try:
        ...
        # MODIFIED: Return dict for error
        return {"error": f"⚠️ Грешка при четене: {str(e)}"}
...
def echo_weaver_commit(file_path: str, content: str, commit_message: str):
    ...
    try:
        ...
        # MODIFIED: Return dict for error
        return {"error": f"⚠️ Грешка в Тъкача: {str(e)}"}
...
def deep_scan_resilient(query: str):
    ...
    try:
        ...
        # MODIFIED: Return dict for error
        return {"error": "Няма сигнал от Скенера."}
...
class ChatSessionManager:
    def __init__(self):
        ...
        # REMOVED: st.session_state.current_session_file_name initialization moved globally

    def list_sessions(self):
        try:
            # MODIFIED: Call local echo_explorer, expect dict
    ...
    def load_session(self, file_name):
        ...
        try:
            # MODIFIED: Call local echo_reader, expect dict
    ...
    # NEW METHOD: save_current_session
    def save_current_session(self, messages):
        ...
        try:
            ...
            if 'status' in weaver_result:
                ...
                st.session_state.current_session_file_name = file_name_to_save
            else:
...
with st.sidebar:
    ...
    if selected_session == "--- Създай/Продължи нова сесия ---":
        ...
        if st.session_state.current_session_file_name is not None:
            ...
            st.session_state.chat_history_data = []
            st.rerun()
    elif not is_current_session_selected:
...
# GLOBAL INITIALIZATION FOR current_session_file_name
                ...
                with st.spinner("Лобсанг размишлява..."):
                    ...
                    while True:
                        ...
                        for call in function_calls:
                            ...
                            # MODIFIED: Pass res_val directly as it's already a dict
